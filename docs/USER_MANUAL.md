...
`python scripts/run_benchmarks.py --which lj-vs-lammps,lj-nvt,eam-cu,eam-cu-vacancy,polymer --potential <Cu EAM file>
...
`HarmonicBond` vs LAMMPS) and needs no potential file at all.
...
`course/` is a twelve-lecture undergraduate course (L0..L11, locked 1:1 with the twelve chapters)
