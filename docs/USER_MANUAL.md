...
- `lammpskill new --preset lj_melt|eam_fcc|spce_water|bead_spring_chain --out DIR [--steps N] [--n N]`
  writes `DIR/in.lammps` (and `water.data` for `spce_water`, `chain.data` for `bead_spring_chain`).
...
`io.log.read_log` (thermo `one`, `multi` and `yaml` styles, warnings, errors, several runs),
...
`python scripts/run_benchmarks.py --which lj-vs-lammps,lj-nvt,eam-cu,eam-cu-vacancy,polymer,water --potential <Cu EAM file>
[--steps N] [--state-point KEY] [--outdir DIR] [--workdir DIR] [--log-dir DIR]`
(`references/benchmarks.md`). `eam-cu-vacancy` cross-checks the vacancy formation energy (mdlite
FIRE-relaxed vs LAMMPS `minimize`) on the same Cu potential and fitted lattice constant as `eam-cu`;
`polymer` cross-checks a bead-spring chain's energy and forces (mdlite's `LennardJones` +
`HarmonicBond` vs LAMMPS) and needs no potential file at all; `water` checks SPC/E's density
(LAMMPS `fix npt` only — `mdlite` has no PPPM/Ewald electrostatics or rigid-body constraints) against
a NIST reference and likewise needs no potential file.

**Running LAMMPS's own examples.** `python scripts/run_examples.py --route <name> --set bench|examples
--root <installed LAMMPS tree> [--procs N] [--time-limit S] [--only TEXT] [--outdir DIR]
[--run-root DIR] [--potentials DIR]` copies each shipped case out of the installed tree into a scratch directory, runs
it on that route, and writes a JSON record plus a markdown table. It scores on the log, never on the
return code — both builds on the reference machine exit 0 after a fatal input error. A case whose
build lacks a package is recorded as `missing-package` with the package named, not as a failure, so
comparing two routes stays honest; where upstream ships a reference log for that process count, the
...
`course/` is a thirteen-lecture undergraduate course (L0..L12, locked 1:1 with the thirteen chapters)
