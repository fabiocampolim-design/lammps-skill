...
def test_polymer_energy_forces_mdlite_matches_a_numerical_derivative():
    ...
    cells alike) is held to. Scope: this is a self-consistency check of mdlite's own force
    against its own energy -- it would catch a sign error or a wrong coefficient in either the
    bond or the LJ term, but not a whole term silently dropped from both E and F together (e.g.
    the LJ contribution removed entirely). That case is caught only by the LAMMPS-backed
    data/records/polymer_vs_lammps.json record, regenerated by the project owner."""
