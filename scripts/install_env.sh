...
# Create the conda env `lammps` and register its Jupyter kernel (Linux/macOS/WSL/Git Bash).
# Usage: install_env.sh [--dry-run] [--conda /path/to/conda]
...
HERE="$(cd "$(dirname "$0")" && pwd)"; YML="$HERE/../environment-lammps.yml"
status=()
step() { echo "== $1"; echo "   $2"; if [ $DRY = 1 ]; then status+=("$1 : DRY-RUN"); return; fi
         if bash -c "$2"; then status+=("$1 : OK"); else status+=("$1 : FAIL"); printf '%s\n' "${status[@]}"; exit 1; fi; }
[ $DRY = 1 ] || command -v "$CONDA" >/dev/null 2>&1 || [ -x "$CONDA" ] || { echo "FAIL: conda not found ($CONDA)"; exit 1; }
...
printf '%s\n' "${status[@]}"
echo "LAMMPS itself: scripts/install_lammps_wsl.sh (apt or --source) inside WSL as root, or any route in references/install-routes.md."
